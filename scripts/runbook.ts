...
const RUNBOOKS: Record<number, EpisodeRunbook> = {
  ...
  2: {
    ...
    loop: [
      ...
      "",
      "# ── run CLI visibility step (see below) ──",
      "# ── apply minimal fix in your IDE (titl → title) ──",
      "# ── or: pnpm ep2:fix   (pre-built annotated fix) ──",
      "",
  ...
  3: {
    ...
    loop: [
      ...
      "",
      "# ── run CLI visibility step (see below) ──",
      "# ── restore .select() + .throwOnError() in your IDE ──",
      "# ── or: pnpm ep3:fix   (pre-built annotated fix) ──",
      "",
  ...
  4: {
    ...
    loop: [
      ...
      "",
      "# ── run CLI visibility step (see below) ──",
      "# ── re-create INSERT policy in your IDE ──",
      "# ── or: pnpm ep4:fix   (pre-built annotated fix) ──",
      "",
      "pnpm ep4:run            # anon still BLOCKED — expected (see prompt note)",
      "pnpm ep4:verify         # confirms all 3 scenarios",
  ...
  5: {
    ...
    loop: [
      ...
      "",
      "# ── run CLI visibility step (see below) ──",
      "# ── or: pnpm ep5:fix   (regenerates types.gen.ts) ──",
      "",
